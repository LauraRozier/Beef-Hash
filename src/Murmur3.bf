		...
		*/
		...
		public static uint8* Hash_x86_128(void* key, int length, uint32 seed)
		{
			...
			switch(len & 15)
			{
				...
				case 13: k4 ^= ((uint32)tail[12]) << 0; k4 *= c4; k4 = rotl32!(k4, 18); k4 *= c1; h4 ^= k4;
				case 12: k3 ^= ((uint32)tail[11]) << 24;
				case 11: k3 ^= ((uint32)tail[10]) << 16;
				case 10: k3 ^= ((uint32)tail[ 9]) << 8;
				case  9: k3 ^= ((uint32)tail[ 8]) << 0; k3 *= c3; k3 = rotl32!(k3, 17); k3 *= c4; h3 ^= k3;
				case  8: k2 ^= ((uint32)tail[ 7]) << 24;
				case  7: k2 ^= ((uint32)tail[ 6]) << 16;
				case  6: k2 ^= ((uint32)tail[ 5]) << 8;
				case  5: k2 ^= ((uint32)tail[ 4]) << 0; k2 *= c2; k2 = rotl32!(k2, 16); k2 *= c3; h2 ^= k2;
		...
		public static uint8* Hash_x64_128(void* key, int length, uint32 seed)
		{
			...
			switch(len & 15)
			{
				...
				case  9: k2 ^= ((uint64)tail[ 8]) << 0; k2 *= c2; k2 = rotl64!(k2, 33); k2 *= c1; h2 ^= k2;
